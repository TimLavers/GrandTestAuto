...
public abstract class PackagesInfo<T extends ClassFinder> {
    ...
    public PackagesInfo(Filter filter, File classesRoot) {
        if (!(classesRoot.exists() && classesRoot.isDirectory())) {
            throw new IllegalArgumentException("Not an existing directory: " + classesRoot);
        }
        this.filter = filter;
     ...
     */
    public List<String> testablePackageNames() {
        List<String> result = new LinkedList<String>();
        result.addAll(packageNamesToInfo.keySet());
        return result;
    }
